#Script for plotting velocity model on map: P
...
#Plot velocity model	
awk '{ if ( $3==1 ) print $1,$2,$5+$4}' $modelpath/velmodel_coord_p.dat | gmt blockmean $bound_map -I.005/.005 -C > temp1.xyz
gmt surface temp1.xyz  -I0.002/0.002   $bound_map -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/temper.cpt -I  -M -S5.1/6.6/0.02 > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_map $proj_map -O -K  >> $output

#Masking	
awk '{ if ( $3==1 && $4>0.66) print $1,$2,$4}' $modelpath/semblance_average_chk20_coord_comb.dat | gmt psmask  $bound_map -I.1m/.1m -S4k  $proj_map  -O -K -N -Gwhite >>$output
...
#Plot topography with tranparency
...
gmt psbasemap  $proj_map $bound_map  -LjRB+c27S+w40k+l+ab+f+o0.7c -P -K -O >> $output
gmt psscale $proj_map $bound_map -Dx4.2/-1.0+w4.2/0.35+h+e -Ctomo.cpt -Ba0.3+l"Vp(km/s)" -G5.1/6.6 -O -K  >> $output	
