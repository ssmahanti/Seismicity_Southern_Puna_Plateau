...
output="velmodel_cross_v1_p.ps"

crange="5.1/6.62/0.02"
...
#gmt psscale -Dx8/0.1+w1.6/0.3+e -Ctomo.cpt -Ba5+l"dVp(%)" -G-10/10 -O -K  >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$4,$5}'  $modelpath/velmodel_coord_p.dat| gmt project -C$A2 -E$B2 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{print $6,-$3,$4+$5}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/temper.cpt -I  -M -S$crange > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
awk '{print $3,4.1}' temp_sa.txt | gmt psxy $proj_cross $bound_cross $Bcross -Ss0.12  -Gfirebrick1 -P  -W0.08p -O -K >> $output

#Plotting VOLCANO
awk '{print $2,$1,$3}' $plotdatapath/holocene_volcano.txt > temp_volcano.txt
awk '{print $2,$1,$3}' $plotdatapath/pliestocene_volcano.txt >> temp_volcano.txt
gmt project temp_volcano.txt -C$A2 -E$B2  -W-5/5 -Fxyzpqrs -Lw -Q  > temp_galloc.txt
awk '{print $4, $3/1000.0}' temp_galloc.txt | gmt psxy $proj_cross $bound_cross -St0.14 -Gmagenta -W0.01,black -O -K    >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$4,$5}'  $modelpath/velmodel_coord_p.dat | gmt project -C$A3 -E$B3 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{ print $6,-$3,$4+$5}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/temper.cpt -I  -M -S$crange > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$4,$5}' $modelpath/velmodel_coord_p.dat | gmt project -C$A4 -E$B4 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{ print $6,-$3,$4+$5}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/temper.cpt -I  -M -S$crange > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$4,$5}' $modelpath/velmodel_coord_p.dat | gmt project -C$A5 -E$B5 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{ print $6,-$3,$4+$5}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/temper.cpt -I  -M -S$crange > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
#COLOR SCALE
gmt psscale -Dx2.0/-1+w3.2/0.3+h+e -Ctomo.cpt -Ba0.3+l"Vp(km/s)" -G5.1/6.6 -O -K  >> $output
