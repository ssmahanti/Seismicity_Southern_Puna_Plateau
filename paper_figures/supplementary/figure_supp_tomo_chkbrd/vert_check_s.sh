#Script to plot checkerboards in cross-sections: S
...
output="checkerboard_20_cross_s.ps"

gmt makecpt -T-10/10/1 -C$cptpath/tomo_lotos.cpt -I -M > tomo.cp
...
awk '{print $1,$2,$3,$6}'  $modelpath/checkerboard_20_coord_s.dat | gmt project -C$A1 -E$B1 -Fxyzpqrs -W$width  -Lw -Q  > temp_tomo.txt
...
#input contour
awk '{print $1,$2,$3,$4}'  $modelpath/checkerboard_20_coord_input.dat | gmt project -C$A1 -E$B1 -Fxyzpqrs -W$width  -Lw -Q  > temp_sem.txt
...
#contour
awk '{print $1,$2,$3,$4}'  $modelpath/semblance_average_chk20_coord_comb.dat | gmt project -C$A1 -E$B1 -Fxyzpqrs -W$width -Lw -Q  > temp_sem.txt
awk '{print $5,-$3,$4}' temp_sem.txt | gmt xyz2grd  -I4/1 -Gtemp_sem.grd $bound_cross
...
#Plotting velocity model	
awk '{print $1,$2,$3,$6}'  $modelpath/checkerboard_20_coord_s.dat | gmt project -C$A2 -E$B2 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{print $5,-$3,$4}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/tomo_lotos.cpt  -M -S-10/10/1 > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
awk '{print $2,$1,$3}' $plotdatapath/holocene_volcano.txt > temp_volcano.txt
awk '{print $2,$1,$3}' $plotdatapath/pliestocene_volcano.txt >> temp_volcano.txt
gmt project temp_volcano.txt -C$A2 -E$B2  -W-10/10 -Fxyzpqrs -Lw -Q  > temp_galloc.txt
awk '{print $4, $3/1000.0}' temp_galloc.txt | gmt psxy $proj_cross $bound_cross -St0.2 -Gmagenta -W0.01,black -O -K    >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$6}'  $modelpath/checkerboard_20_coord_s.dat | gmt project -C$A3 -E$B3 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
...
awk '{print $2,$1,$3}' $plotdatapath/holocene_volcano.txt > temp_volcano.txt
awk '{print $2,$1,$3}' $plotdatapath/pliestocene_volcano.txt >> temp_volcano.txt
gmt project temp_volcano.txt -C$A3 -E$B3 -Fxyzpqrs -W-10/10  -Lw -Q  > temp_galloc.txt
awk '{print $4, $3/1000.0}' temp_galloc.txt | gmt psxy $proj_cross $bound_cross -St0.2 -Gmagenta -W0.01,black -O -K    >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$6}' $modelpath/checkerboard_20_coord_s.dat | gmt project -C$A4 -E$B4 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{ print $5,-$3,$4}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/tomo_lotos.cpt  -M -S-10/10/1 > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
#Plotting velocity model	
awk '{print $1,$2,$3,$6}' $modelpath/checkerboard_20_coord_s.dat | gmt project -C$A5 -E$B5 -Fxyzpqrs -W-5/5  -Lw -Q  > temp_tomo.txt
awk '{ print $5,-$3,$4}' temp_tomo.txt  | gmt blockmean $bound_cross2 -I1/1 -C > temp1.xyz
gmt surface temp1.xyz -I0.05/0.05  $bound_cross2 -Gtemp1.grd  
gmt grd2cpt temp1.grd  -C$cptpath/tomo_lotos.cpt  -M -S-10/10/1 > tomo.cpt
gmt grdimage temp1.grd -Ctomo.cpt $bound_cross $proj_cross -O -K  >> $output
...
#########################################
gmt psscale -Dx2.6/-1+w2.4/0.3+h+e -Ctomo.cpt -Ba3+l"dVs(%)" -G-10/10 -O -K  >> $output
