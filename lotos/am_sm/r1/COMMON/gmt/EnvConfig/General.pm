#
# EnvConfig::General.pm - Generic Config Module
#
...
$EnvConfig::General::VERSION = "2.31";
...
sub new {
  #
  # create new EnvConfig::General object
  #
  my($this, @param ) = @_;
  ...
  if ($self->{InterPolateVars} || $self->{InterPolateEnv}) {
    ...
    #
    bless($self, "EnvConfig::General::Interpolated");
    eval {
      require EnvConfig::General::Interpolated;
    };
    if ($@) {
      croak $@;
    }
  ...
  if ($self->{ExtendedAccess}) {
    ...
    # for inheriting the methods available overthere, which we doesn't have.
    #
    bless($self, "EnvConfig::General::Extended");
    eval {
      require EnvConfig::General::Extended;
    };
    if ($@) {
      croak $@;
    }
...
sub ParseConfig {
  ...
  #
  return (new EnvConfig::General(@_))->getall();
}
...
sub SaveConfig {
  ...
  if (!$file || !$hash) {
    ...
    if (ref($hash) ne "HASH") {
      ...
      (new EnvConfig::General(-ConfigHash => $hash))->save_file($file);
    }
  }
}
...
sub SaveConfigString {
  ...
  if (!$hash) {
    ...
    if (ref($hash) ne "HASH") {
      ...
      return (new EnvConfig::General(-ConfigHash => $hash))->save_string();
    }
  }
}
...
EnvConfig::General - Generic Config Module
...
=head1 SYNOPSIS
 ...
 $conf = new EnvConfig::General("rcfile");
...
The format of config files supported by B<EnvConfig::General> is inspired by the well known apache config
...
Possible ways to call B<new()>:
 ...
 $conf = new EnvConfig::General(\%somehash);

 $conf = new EnvConfig::General( %options ); # see below for description of possible options


This method returns a B<EnvConfig::General> object (a hash blessed into "EnvConfig::General" namespace.
...
config files which have to be included. EnvConfig::General will search within this
...
The default behavior of EnvConfig::General is to create an array if some junk in a
...
input. See L<EnvConfig::General::Interpolated> for more informations.
...
access the parsed config. See L<EnvConfig::General::Extended> for more informations.

=item B<-StrictObjects>

By default this is turned on, which causes EnvConfig::General to croak with an
...
You can influence the way how EnvConfig::General decides which part of a line
...
EnvConfig::General is able to notice c-style comments (see section COMMENTS).
...
This is because I<EnvConfig::General> does not
...
The normal behavior of EnvConfig::General is to look for whitespaces in
...
If turned off, EnvConfig::General will complain about multiple occuring options
...
of the line. The EnvConfig::General module will concatenate those lines to one single-value.
...
In this example EnvConfig::General will try to include I<acl.cfg> from I</etc/crypt.d>:

 /etc/crypt.d/acl.cfg

The default behavior (if B<-IncludeRelative> is B<not> set!) will be to open just I<acl.cfg>,
whereever it is, i.e. if you did a chdir("/usr/local/etc"), then EnvConfig::General will include:
...
In this example the value of $config{bgcolor} will be "#ffffcc", EnvConfig::General will not treat
...
Use the module B<EnvConfig::General::Extended>, which is
supplied with the EnvConfig::General distribution.

=head1 VARIABLE INTERPOLATION

You can use variables inside your configfiles if you like. To do
that you have to use the module B<EnvConfig::General::Interpolated>,
which is supplied with the EnvConfig::General distribution.


=head1 EXPORTED FUNCTIONS

EnvConfig::General exports some functions too, which makes it somewhat
...
I recommend you to read the following documentations, which are supplied with perl:
 ...
 EnvConfig::General::Extended      Object oriented interface to parsed configs
 EnvConfig::General::Interpolated  Allows to use variables inside config files
