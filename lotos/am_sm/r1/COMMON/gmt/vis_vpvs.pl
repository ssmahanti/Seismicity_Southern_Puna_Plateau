...
{
    my $SHIFT_TOP = $PAGE_HEIGHT-$MARGIN_TOP+$DY-($YMAP_HOR+$DY)*$N_ROWS_HOR - $SCALE_SHIFT;
    
    my ($title_x,$title_y) = ($MARGIN_LEFT,$SHIFT_TOP - $TITLE_SHIFT_TOP);
    
    my $cmd = "echo \"$title_x $title_y $TITLE_FONT_SIZE 0 1 BL Velocity anomalies in vertical sections. Area : $ar, Model : $md, Iteration : $iter\"";
...
sub main()
{
    ...
    $CONF_FILE_NAME = "../../../DATA/$ar/vpvs.conf";
