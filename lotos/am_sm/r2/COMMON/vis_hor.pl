...
sub main()
{
    ...
    $ps_out = "../../../PICS/ps/$ar/$md/hor_nodes.ps";
	...
	for my $j (0..$N_COLUMNS-1)
	{
	    ...
	    ($xyz_out,$title) = split(/\|/,$info);
	    if ($i+1 == $N_ROWS)
	    {
		if ($j == 0)
		{
		    draw_map($xyz_out,$title,$MARGIN_LEFT+($XMAP+$DX)*$j,$PAGE_HEIGHT-$MARGIN_TOP+$DY-($YMAP+$DY)*($i+1),3);
		}
		else
		{
		    draw_map($xyz_out,$title,$MARGIN_LEFT+($XMAP+$DX)*$j,$PAGE_HEIGHT-$MARGIN_TOP+$DY-($YMAP+$DY)*($i+1),2);
		}
	    }
	    elsif($j == 0)
	    {
		draw_map($xyz_out,$title,$MARGIN_LEFT+($XMAP+$DX)*$j,$PAGE_HEIGHT-$MARGIN_TOP+$DY-($YMAP+$DY)*($i+1),1);
	    }
	    else
	    {
		draw_map($xyz_out,$title,$MARGIN_LEFT+($XMAP+$DX)*$j,$PAGE_HEIGHT-$MARGIN_TOP+$DY-($YMAP+$DY)*($i+1),0);
	    }
	}
    }
...
sub draw_map($$$$$)
{
    ...
    my ($xyz_out,$title,$OFF_X,$OFF_Y,$axis) = @_;
