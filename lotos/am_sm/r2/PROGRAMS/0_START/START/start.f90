...
do iar=1,n_ar
    ...
    i=system('mkdir -p ../../../DATA/'//ar//'/'//md//'/data')
    ...
    do iter=1,niter	
        ...
        do igr=1,nornt
            ...
            if(iter.eq.1) then
                ...
                i=system('../../2_INVERS_3D/4n_tetrad/Tetrad.exe')
                i=system('../../2_INVERS_3D/5n_sosedi/add_matr.exe')
                !write(*,*)' _________________________________________________________'
                !write(*,*)' VISUALIZE THE RAY PATHS AND GRID IN HORIZONTAL AND VERTICAL SECTIONS '
                !i=system('../../3_VISUAL/_vis_ray_path/paths.exe')
            end if

            if(VPSX_key.eq.1) then
                write(*,*)' _________________________________________________________'
                write(*,*)' COMPUTE THE 1ST DERIVATIVE MATRIX (VP-VS SCHEME) '
                i=system('../../2_INVERS_3D/6n_matr/matr.exe')
                write(*,*)' _________________________________________________________'
                write(*,*)' PERFORM THE INVERSION (VP-VS SCHEME) '
                i=system('../../2_INVERS_3D/7n_invers/Invbig.exe')
            else if(VPSX_key.eq.2) then
                write(*,*)' _________________________________________________________'
                write(*,*)' COMPUTE THE 1ST DERIVATIVE MATRIX (VP-VP/VS SCHEME) '
                i=system('../../2_INVERS_3D/6n_matr_Vp_X/matr.exe')
                write(*,*)' _________________________________________________________'
                write(*,*)' PERFORM THE INVERSION (VP-VP/VS SCHEME) '
                i=system('../../2_INVERS_3D/7n_invers_Vp_X/Invbig.exe')
            else
                write(*,*)' VPSX_key=',VPSX_key
                write(*,*)' for node parameterization this key is not allowed'
                pause
            end if
        end do
        ...
        if(VPSX_key.eq.1) then
	        ...
	        i=system('../../2_INVERS_3D/8n_3D_model/mod_3D.exe')
                write(*,*)' _________________________________________________________'
	        ...
	        i=system('../../3_VISUAL/_vis_n_ver_result/visual.exe')
                ...
                write(*,*)' _________________________________________________________'
                write(*,*)' COMPUTE THE VELOCITY FIELS IN 3D REGILAR GRID (VP-VP/VS SCHEME) '
	        i=system('../../2_INVERS_3D/8n_3D_model_Vp_X/mod_3D.exe')
                write(*,*)' _________________________________________________________'
                write(*,*)' VISUALIZE THE RESULT IN HORIZONTAL SECTIONS (VP-VP/VS SCHEME) '
	        i=system('../../3_VISUAL/_vis_n_hor_vpvs/visual.exe')
                write(*,*)' _________________________________________________________'
                write(*,*)' VISUALIZE THE RESULT IN VERTICAL SECTIONS (VP-VP/VS SCHEME) '
	        i=system('../../3_VISUAL/_vis_n_ver_vpvs/visual.exe')
        end if
