...
!Applying sin function for checkerboards by ssmahanti

xn=(xx-xbr1(ips))/dxbr2(ips)
yn=(yy-ybr1(ips))/dybr2(ips)
zn=(zz-zbr1(ips))/dzbr2(ips)
tpx=2*dxbr1(ips)/dxbr2(ips)
tpy=2*dybr1(ips)/dybr2(ips)
tpz=2*dzbr1(ips)/dzbr2(ips)
!dv_board=sign*amp(ips)
dv_board=sin(2*3.1415*xn/tpx)*sin(2*3.1415*yn/tpy)*sin(2*3.1415*zn/tpz)*amp(ips)
!write(*,*)xx,yy,zz,sin(2*3.1415*xn/tpx),sin(2*3.1415*yn/tpy),sin(2*3.1415*zn/tpz),dv_board
return
end