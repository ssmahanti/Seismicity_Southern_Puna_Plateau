...
%--------------------------------------------------------------------------
figure; hold on;
%title('Shape ratio','FontSize',14); 
hist(shape_ratio_statistics,shape_ratio_axis);
xlabel("Shape Ratio",'FontSize',25)
ylabel("Count",'FontSize',25)
v = axis; axis([shape_ratio_min shape_ratio_max v(3) v(4)+2]);
box on;
h = gca; set(h,'FontSize',25,'XGrid','on','YGrid','on','LineWidth',1.5);

% saving the plot
saveas(gcf,shape_ratio_plot,'pdf');
