...
build_taup_model("itvel2.nd",output_folder=".") 
...
model = TauPyModel(model="itvel2")

dist=2.0 #dist range in deg.
dep=50 #depth in km

ddist=0.1 #dist interval, be exactly divided by dist
ddep=1.0 #depth interval, be exactly divided by dep

ndep=int(dep/ddep)+1
ndist=int(dist/ddist)+1

with open("ttdb_puna_new.txt", "w") as f:
