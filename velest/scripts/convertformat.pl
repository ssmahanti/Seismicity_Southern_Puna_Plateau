#!/usr/bin/perl -w
$station = "../spuna_stations.txt";

$stause = "../spuna.sta"; #station format for VELEST
open(JK,"<$station");
@par = <JK>;
close(JK);
...
foreach $_(@par){
	...
	printf OT "%-6s%7.4f%1s %8.4f%s %4d %1d %3d %5.2f  %5.2f %1d\n",$net.$sta,$lat,$vsn,$lon,$vew,$p4,$p2,$p3,$v1,$v2,1;
	$p3++;
}
print OT "\n";
close(OT);
...
$phaseout = "../puna_crust.pha"; # phase format for VELEST ISED=1
$phasecat = "../puna_crust.cat"; # catalog format for VELEST
...
#		print $num
		...
		print EV "\n";
		printf EV "%02d%02d%02d %2d%2d %5.2f %7.4f%s %8.4f%s %7.2f  %5.2f\n",$year,$month,$day,$hour,$min,$sec,$lat,$vsn,$lon,$vew,$dep,$mag;
		printf CT "%02d%02d%02d %2d%2d %5.2f %7.4f%s %8.4f%s %7.2f  %5.2f\n",$year,$month,$day,$hour,$min,$sec,$lat,$vsn,$lon,$vew,$dep,$mag;
	}else{
